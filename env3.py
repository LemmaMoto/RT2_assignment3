...
class TwoColoursAssignmentArena(Arena):
    ...
    def __init__(self, objects=None, wall_markers=False):
        ...
        def place_token_randomly(number_of_tokens):
            for i in range(number_of_tokens):
                token = GoldToken(self, i)  # Only create GoldToken objects
                # Generate random location within the arena
                token.location = (random.uniform(-self.size[0] / 2, self.size[0] / 2), 
                                random.uniform(-self.size[1] / 2, self.size[1] / 2))
                token.heading = random.uniform(0, 2 * pi)  # Random orientation
                self.objects.append(token)

        place_token_randomly(TOKENS_PER_CIRCLE)  # Adjust the number of tokens as needed  
